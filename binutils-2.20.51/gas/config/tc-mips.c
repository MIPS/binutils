...
is_opcode_valid (const struct mips_opcode *mo)
{
  ...
  if (mips_opts.micromips)
    {
      if (mips_opts.insn32 == TRUE)
	{
	  if (mo->pinfo != INSN_MACRO && !is_micromips_32bit_p (mo))
	    return FALSE;
	  if (mo->pinfo2 & INSN2_BRANCH_DELAY_16BIT)
	    return FALSE;
	}
...
macro_build_jalr (expressionS *ep)
{
  ...
  if (mips_opts.micromips)
    {
      if (HAVE_NEWABI || mips_opts.insn32)
...
macro (struct mips_cl_insn *ip)
{
  ...
  switch (mask)
	...
	abort ();
      ...
      macro_build (NULL, s, "\\,~(b)", (int) EXTRACT_OPERAND (3BITPOS, *ip),
		   (unsigned long) 0, AT);
      break;
...
{
  static symbolS *saved_addsy;
  static valueT saved_offset;
  static fixS *saved_fixp;
  symbolS *prev_addsy;
  valueT prev_offset;
  fixS *prev_fixp;

  prev_offset = saved_offset;
  prev_addsy = saved_addsy;
  prev_fixp = saved_fixp;
  saved_offset = fixp->fx_offset;
  saved_addsy = fixp->fx_addsy;
  saved_fixp = fixp;
  ...
  if (excess != 0)
    {
      ...
      if (excess < 2 || *p != 2 || mips_opts.insn32)
	memset (p, 0, excess);
      else
	{
	  memset (p, 0, excess - 2);
  ...
  for (;;)
      ...
      for (args = insn->args;; ++args)
	{
	  ...
	  switch (*args)
	    ...
	    case '2': /* dsp 2-bit unsigned immediate in bit 14 */
	      my_getExpression (&imm_expr, s);
	      check_absolute_expr (ip, &imm_expr);
	      if ((unsigned long) imm_expr.X_add_number != 1
		  && (unsigned long) imm_expr.X_add_number != 3)
		{
	      ...
	      MICROMIPS_INSERT_OPERAND (BP, *ip, imm_expr.X_add_number);
	      imm_expr.X_op = O_absent;
	      s = expr_end;
	      continue;

	    case '3': /* dsp 3-bit unsigned immediate in bit 13 */
	      my_getExpression (&imm_expr, s);
	      check_absolute_expr (ip, &imm_expr);
	      if (imm_expr.X_add_number & ~MICROMIPSOP_MASK_SA3)
		{
		  as_bad (_("DSP immediate not in range 0..%d (%lu)"),
			  MICROMIPSOP_MASK_SA3,
			  (unsigned long) imm_expr.X_add_number);
		}
	      MICROMIPS_INSERT_OPERAND (SA3, *ip, imm_expr.X_add_number);
	      imm_expr.X_op = O_absent;
	      s = expr_end;
	      continue;

	    case '4': /* dsp 4-bit unsigned immediate in bit 12 */
	      my_getExpression (&imm_expr, s);
	      check_absolute_expr (ip, &imm_expr);
	      if (imm_expr.X_add_number & ~MICROMIPSOP_MASK_SA4)
		{
		  as_bad (_("DSP immediate not in range 0..%d (%lu)"),
			  MICROMIPSOP_MASK_SA4,
			  (unsigned long) imm_expr.X_add_number);
		}
	      MICROMIPS_INSERT_OPERAND (SA4, *ip, imm_expr.X_add_number);
	      imm_expr.X_op = O_absent;
	      s = expr_end;
	      continue;

	    case '5': /* dsp 8-bit unsigned immediate in bit 13 */
	      my_getExpression (&imm_expr, s);
	      check_absolute_expr (ip, &imm_expr);
	      if (imm_expr.X_add_number & ~MICROMIPSOP_MASK_IMM8)
		{
		  as_bad (_("DSP immediate not in range 0..%d (%lu)"),
			  MICROMIPSOP_MASK_IMM8,
			  (unsigned long) imm_expr.X_add_number);
		}
	      MICROMIPS_INSERT_OPERAND (IMM8, *ip, imm_expr.X_add_number);
	      imm_expr.X_op = O_absent;
	      s = expr_end;
	      continue;

	    case '6': /* dsp 5-bit unsigned immediate in bit 16 */
	      my_getExpression (&imm_expr, s);
	      check_absolute_expr (ip, &imm_expr);
	      if (imm_expr.X_add_number & ~MICROMIPSOP_MASK_RS)
		{
		  as_bad (_("DSP immediate not in range 0..%d (%lu)"),
			  MICROMIPSOP_MASK_RS,
			  (unsigned long) imm_expr.X_add_number);
		}
	      MICROMIPS_INSERT_OPERAND (RS, *ip, imm_expr.X_add_number);
	      imm_expr.X_op = O_absent;
	      s = expr_end;
	      continue;
		  ...
		  && s[3] >= '0' && s[3] <= '3')
		{
		  regno = s[3] - '0';
		  s += 4;
		  MICROMIPS_INSERT_OPERAND (DSPACC, *ip, regno);
		  continue;
		}
	      else
		as_bad (_("Invalid dsp acc register"));
	    ...
	    case '8': /* dsp 6-bit unsigned immediate in bit 14 */
	      my_getExpression (&imm_expr, s);
	      check_absolute_expr (ip, &imm_expr);
	      if (imm_expr.X_add_number & ~MICROMIPSOP_MASK_WRDSP)
		{
		  as_bad (_("DSP immediate not in range 0..%d (%lu)"),
			  MICROMIPSOP_MASK_WRDSP,
			  (unsigned long) imm_expr.X_add_number);
		}
	      MICROMIPS_INSERT_OPERAND (WRDSP, *ip, imm_expr.X_add_number);
	      imm_expr.X_op = O_absent;
	      s = expr_end;
	      continue;

	    case '9': /* dsp 5-bit unsigned immediate in bit 11 */
	      my_getExpression (&imm_expr, s);
	      check_absolute_expr (ip, &imm_expr);
	      if (imm_expr.X_add_number & ~MICROMIPSOP_MASK_RD)
		{
	      ...
	      MICROMIPS_INSERT_OPERAND (RD, *ip, imm_expr.X_add_number);
	      imm_expr.X_op = O_absent;
	      s = expr_end;
	      continue;
	    ...
	    case '0': /* dsp 6-bit signed immediate in bit 16 */
	      my_getExpression (&imm_expr, s);
	      check_absolute_expr (ip, &imm_expr);
	      min_range = -((MICROMIPSOP_MASK_DSPSFT + 1) >> 1);
	      max_range = ((MICROMIPSOP_MASK_DSPSFT + 1) >> 1) - 1;
	      if (imm_expr.X_add_number < min_range
		  || imm_expr.X_add_number > max_range)
		{
		  as_bad (_("DSP immediate not in range %ld..%ld (%ld)"),
	      ...
	      MICROMIPS_INSERT_OPERAND (DSPSFT, *ip, imm_expr.X_add_number);
	      imm_expr.X_op = O_absent;
	      s = expr_end;
	      continue;
	    ...
	    case '@': /* dsp 10-bit signed immediate in bit 16 */
	      my_getExpression (&imm_expr, s);
	      check_absolute_expr (ip, &imm_expr);
	      min_range = -((MICROMIPSOP_MASK_IMM10 + 1) >> 1);
	      max_range = ((MICROMIPSOP_MASK_IMM10 + 1) >> 1) - 1;
	      if (imm_expr.X_add_number < min_range
		  || imm_expr.X_add_number > max_range)
		{
		  as_bad (_("DSP immediate not in range %ld..%ld (%ld)"),
	      ...
	      MICROMIPS_INSERT_OPERAND (IMM10, *ip, imm_expr.X_add_number);
	      imm_expr.X_op = O_absent;
	      s = expr_end;
	      continue;
		       ...
		       va_list args)
{
  ...
  for (;;)
	...
	case '2':
	  MICROMIPS_INSERT_OPERAND (BP, insn, va_arg (args, int));
	  continue;
	...
	case 'c':
	  MICROMIPS_INSERT_OPERAND (BREAKCODE, insn, va_arg (args, int));
	  continue;
  ...
  switch (mask)
      ...
      if (treg == 0)
	{
	  ...
	  if (mips_trap)
	    ...
	    macro_build (NULL, "break", mips_opts.insn32 ? "c" : "mF", 7);
	  break;
	}
      ...
      else
	{
	  ...
	  macro_build (NULL, "break", mips_opts.insn32 ? "c" : "mF", 6);
	}
      micromips_add_number_label ();
      macro_build (NULL, s, mips_opts.insn32 ? "s" : "mj", dreg);
      break;
	  ...
	  && s[strlen (s) - 1] != 'u')
	{
      ...
      load_register (AT, &imm_expr, dbl);
      macro_build (NULL, s, "z,s,t", sreg, AT);
      macro_build (NULL, s2, mips_opts.insn32 ? "s" : "mj", dreg);
      break;
    ...
    case M_JRADDIUSP:
      ...
      expr1.X_add_number = MICROMIPS_EXTRACT_OPERAND (IMMP, *ip) << 2;
      macro_build (&expr1, "addiu", "t,r,j", SP, SP, BFD_RELOC_MICROMIPS_LO16);
      end_noreorder ();
      break;
    ...
    case M_MOVEP:
      gas_assert (mips_opts.micromips);
      gas_assert (mips_opts.insn32);
      dreg = micromips_to_32_reg_h_map[MICROMIPS_EXTRACT_OPERAND (MH, *ip)];
      breg = micromips_to_32_reg_i_map[MICROMIPS_EXTRACT_OPERAND (MI, *ip)];
      sreg = micromips_to_32_reg_m_map[MICROMIPS_EXTRACT_OPERAND (MM, *ip)];
      treg = micromips_to_32_reg_n_map[MICROMIPS_EXTRACT_OPERAND (MN, *ip)];
      move_register (dreg, sreg);
    ...
    case M_MUL:
      macro_build (NULL, dbl ? "dmultu" : "multu", "s,t", sreg, treg);
      macro_build (NULL, "mflo", mips_opts.insn32 ? "s" : "mj", dreg);
      break;
	...
	load_register (AT, &imm_expr, dbl);
      ...
      macro_build (NULL, "mfhi", mips_opts.insn32 ? "s" : "mj", AT);
      if (mips_trap)
	macro_build (NULL, "tne", "s,t,q", dreg, AT, 6);
      else
	{
