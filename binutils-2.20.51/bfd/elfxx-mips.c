		    ...
		    struct mips_elf_link_hash_entry *h, bfd_vma symbol)
{
  ...
  if (r_type == R_MIPS_TLS_GOTTPREL || r_type == R_MIPS16_TLS_GOTTPREL
      || r_type == R_MICROMIPS_TLS_GOTTPREL)
    {
  ...
  if (r_type == R_MIPS_TLS_GD || r_type == R_MIPS16_TLS_GD
      || r_type == R_MICROMIPS_TLS_GD)
    {
  ...
  if (r_type == R_MIPS_TLS_LDM || r_type == R_MIPS16_TLS_LDM
      || r_type == R_MICROMIPS_TLS_LDM)
    {
	  ...
	  if (is_mips16)
	    {
	      ...
	      _bfd_mips16_elf_reloc_shuffle (output_bfd, R_MIPS16_HI16, FALSE,
					     loc);
	      _bfd_mips16_elf_reloc_shuffle (output_bfd, R_MIPS16_LO16, FALSE,
					     loc + 8);
	    }
	  else if (htab->insn32)
	    {
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
      ...
      if (r_type == R_MICROMIPS_HI16 && MATCH (opcode, lui_insn))
	{
	  ...
	  if (r_offset >= 2
	      ...
	      && !(r_offset >= 4
		   && (bzc = check_bzc (abfd, ptr - 4, r_offset - 4,
  ...
  for (plt_offset = plt0_size;
      ...
      if (opcode == 0xf4003240)
	{
	  if ((e_flags & EF_MIPS_ARCH_ASE_MICROMIPS) != 0)
	    return -1;
	  _bfd_mips16_elf_reloc_unshuffle (abfd, R_MIPS16_HI16, FALSE,
					   plt_data + plt_offset);
	  _bfd_mips16_elf_reloc_unshuffle (abfd, R_MIPS16_LO16, FALSE,
	  ...
	  _bfd_mips16_elf_reloc_shuffle (abfd, R_MIPS16_HI16, FALSE,
					 plt_data + plt_offset);
	  _bfd_mips16_elf_reloc_shuffle (abfd, R_MIPS16_LO16, FALSE,
					 plt_data + plt_offset + 8);
